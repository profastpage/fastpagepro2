  ...
  return (
    ...
    <div ref={containerRef} className={`min-h-screen font-sans overflow-x-hidden transition-colors duration-500 ${isDarkMode ? 'dark' : ''}`}>
      ...
      <nav className="fixed w-full z-50 transition-all duration-700 h-[60px] sm:h-[64px]" style={{
        background: scrolled
          ? (isDarkMode ? 'rgba(0, 0, 0, 0.55)' : 'rgba(255, 255, 255, 0.65)')
          : 'rgba(0, 0, 0, 0.55)',
        ...
        boxShadow: scrolled
          ? (isDarkMode ? '0 2px 20px rgba(0, 0, 0, 0.4)' : '0 4px 30px rgba(0, 0, 0, 0.08)')
          : '0 2px 20px rgba(0, 0, 0, 0.4)'
      }}>
        <div className="container mx-auto px-4 md:px-6 h-full flex justify-between items-center">
          <motion.div
            className={`font-bold text-xl tracking-tighter cursor-pointer flex items-center gap-2.5 transition-colors duration-500 ${scrolled && !isDarkMode ? 'text-zinc-950' : 'text-white'}`}
          ...
          <div className="hidden md:flex gap-1 items-center">
            {navItems.map((item) => (
              <motion.a key={item.id} href={`#${item.id}`} onClick={(e) => scrollToSection(e, item.id)} whileHover={{ y: -2 }} className={`nav-link text-sm font-semibold transition-colors px-4 py-2 rounded-lg ${scrolled && !isDarkMode ? 'text-zinc-900 hover:text-zinc-950 hover:bg-black/5' : 'text-white hover:text-yellow-400 hover:bg-white/10'}`}>{item.label}</motion.a>
            ))}
            <div className={`w-px h-5 mx-2 ${scrolled && !isDarkMode ? 'bg-black/10' : 'bg-white/15'}`} />
            {/* Language Toggle */}
            <motion.button
              onClick={toggleLanguage}
              whileHover={{ scale: 1.06 }}
              whileTap={{ scale: 0.94 }}
              className={`nav-icon-btn px-3 py-2 text-xs font-bold transition-colors rounded-lg ${scrolled && !isDarkMode ? 'text-zinc-800 hover:text-amber-600' : 'text-white hover:text-yellow-400'}`}
          ...
          <div className="md:hidden flex items-center gap-2">
            <motion.button
              onClick={toggleLanguage}
              whileTap={{ scale: 0.94 }}
              className={`nav-icon-btn px-2.5 py-2 text-[10px] font-bold transition-colors rounded-lg ${scrolled && !isDarkMode ? 'text-zinc-800 hover:text-amber-600' : 'text-white hover:text-yellow-400'}`}
              aria-label="Toggle language"
            >
              {language === 'es' ? 'EN' : 'ES'}
            </motion.button>
            <ThemeToggle isDark={isDarkMode} toggleTheme={toggleTheme} />
            <motion.button onClick={() => setMobileMenu(!mobileMenu)} whileTap={{ scale: 0.9 }} className={`nav-icon-btn w-9 h-9 flex items-center justify-center rounded-lg transition-colors ${scrolled && !isDarkMode ? 'text-zinc-800 hover:text-zinc-950' : 'text-white hover:text-yellow-400'}`}>
